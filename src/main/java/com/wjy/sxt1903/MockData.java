...
//     	String[] areas = new String[]{"海淀区","朝阳区","昌平区","东城区","西城区","丰台区","顺义区","大兴区"};
    	...
    	for (int i = 0; i < 3000; i++) {
    		//模拟车牌号：如：京A00001
        	String car = locations[random.nextInt(10)] + (char)(65+random.nextInt(26))+MyStringUtils.fulfuill(5,random.nextInt(100000)+"");
        	
        	//baseActionTime 模拟24小时
        	String baseActionTime = date + " " + MyStringUtils.fulfuill(random.nextInt(24)+"");//2018-01-01 01
        	/**
        	...
        	for(int j = 0 ; j < (random.nextInt(300)+1) ; j++){
        		...
        		if(j % 30 == 0 && j != 0){
        			 baseActionTime = date + " " + MyStringUtils.fulfuill((Integer.parseInt(baseActionTime.split(" ")[1])+1)+"");
        		}
        		
        		String areaId = MyStringUtils.fulfuill(2,random.nextInt(8)+1+"");//模拟areaId 【一共8个区域】
        		
        		String roadId = random.nextInt(50)+1+"";//模拟道路id 【1~50 个道路】
        		
        		String monitorId = MyStringUtils.fulfuill(4, random.nextInt(9)+"");//模拟9个卡扣monitorId，0补全4位
        		
        		String cameraId = MyStringUtils.fulfuill(5, random.nextInt(100000)+"");//模拟摄像头id cameraId
        		
        		String actionTime = baseActionTime + ":" 
        				+ MyStringUtils.fulfuill(random.nextInt(60)+"") + ":" 
        				+ MyStringUtils.fulfuill(random.nextInt(60)+"");//模拟经过此卡扣开始时间 ，如：2018-01-01 20:09:10
    	...
    	for(Row row : dataList){
    		...
    		index++;
    		if(index % 1000 == 0){
    			sets.add(MyStringUtils.fulfuill(5, random.nextInt(100000)+""));
    		} 
